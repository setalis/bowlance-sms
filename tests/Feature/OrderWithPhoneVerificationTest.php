...
it('требует адрес доставки при выборе доставки', function () {
    ...
    $response->assertStatus(422);
    $response->assertJsonValidationErrors(['delivery_city', 'delivery_street']);
});
...
it('сохраняет delivery_city, delivery_street, delivery_house в адресе пользователя при создании заказа', function () {
    $user = \App\Models\User::factory()->create([
        'phone' => '+995555123456',
    ]);
    ...
    $this->actingAs($user)->postJson('/orders', $orderData)->assertStatus(201);
...
it('заказ с callback имеет корректные флаги phone_verified и needs_callback', function () {
    ...
    $order = \App\Models\Order::where('customer_phone', '+995599000111')->first();

    expect($order->needs_callback)->toBeTrue();
