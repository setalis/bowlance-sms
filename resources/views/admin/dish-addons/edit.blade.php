...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-base-100 shadow-base-300/20 w-full space-y-6 rounded-xl p-6 shadow-md lg:p-8">
            <div>
                <h3 class="text-base-content mb-1.5 text-2xl font-semibold">Редактировать добавку</h3>
            </div>

            <form action="{{ route('admin.dish-addons.update', $addon) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text"
                               ...
                               id="name_ru"
                               value="{{ old('name_ru', $addon->name_ru) }}"
                               required />
                        @error('name_ru')
                    ...
                    <div>
                        ...
                        <input type="text"
                               ...
                               id="name_ka"
                               value="{{ old('name_ka', $addon->name_ka) }}" />
                        @error('name_ka')
                            <span class="text-error text-sm mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
